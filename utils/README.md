# Transcript Extraction Tools - Engineering Documentation

Technical documentation for engineers working with the transcript extraction scripts.
...
- **`extract_transcripts.js`** - Sequential extraction (slow, not recommended)
- **`extract_transcripts_sample.js`** - Test with small sample
- **`extract_transcripts_parallel.js`** - Parallel batch processing (recommended)
- **`merge_transcripts.js`** - Merge batch files into single output

### Instagram Scripts

- **`extract_instagram_deepgram.js`** - Deepgram API extraction
- **`extract_instagram_transcripts.sh`** - Local whisper.cpp extraction
- **`merge_instagram_transcripts.js`** - Merge Instagram batch files
- **`test_deepgram_flux.sh`** - Test Deepgram flux model
- **`test_deepgram_one.js`** - Test single video extraction
...
1. **Extract** - Run extraction scripts to generate transcripts
2. **Merge** - Combine batch files if using parallel processing
3. **Analyze** - Use transcript data for content analysis
4. **Clean** - Remove temporary batch files

### Performance Optimization

- **Parallel processing** reduces TikTok extraction time from ~60 minutes to ~6 minutes
- **Smart skip logic** in Instagram scripts avoids re-processing videos
- **Batch size** can be adjusted in parallel scripts (default: 10 batches)
- **Rate limiting** is handled automatically by Deepgram API
