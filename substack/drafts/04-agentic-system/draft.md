...
2:30am on a Tuesday. I'm watching logs scroll while a user types "analyze CRWD" into Bloom. Twelve seconds later they get back: revenue growing 33% at a $3.4B run rate, operating margins from 1% to 15% over three years, FCF margin above 30%, a bull/bear case, insider activity, and a technical snapshot with RSI and moving average positions.

Four tool calls fired in parallel. `get_symbols_metadata`, `get_price_charts`, `analyze_technicals`, `analyze_earnings` — each hitting a different API, all synthesized into one analysis with an actual opinion on the stock. Not a hedge. Not "it depends." A real take with specific numbers.

That looks simple.

It's the hardest engineering I've done.
...
So I cut. Removed all ten question-type handlers. Cut two of four few-shot examples. Deleted the entire tool-calling policy.
...
Early on I made the mistake every AI engineer makes. One `get_stock_info` function that returned financials, prices, news, technicals, and sentiment in a bloated JSON blob. User asks "what's Apple's price?" and I'm fetching earnings transcripts and insider transactions.

Slow, expensive, and the model couldn't figure out when to use it.

The fix: decomposition. 17 focused tools across three categories.

**Data retrieval** (7 tools): fundamentals/ratings/peers, historical prices, quarterly financials with YoY growth, multi-channel news (earnings, insider trades, M&A, FDA), market sentiment (AAII, Fear & Greed, VIX), market movers/sector breakdown, curated stock lists by theme.

**Analysis** (5 tools): earnings history with transcript insights, technicals (RSI, MAs, chart patterns, options activity, insider transactions), options sentiment over 7-90 days, portfolio risk-return (correlation, drawdown, VaR, Sharpe), trade evaluation against catalyst strength and portfolio fit.

**Research** (5 tools): earnings transcript keyword search with speaker attribution, AI-generated catalyst analysis, quantitative screening with natural language operators (`market_cap > 10B`, `pe_ratio < 25`), web search, URL fetching for anything the specialized tools miss.

Three design decisions I'd make again:

**Batch parameters.** `analyze_technicals` takes up to 5 symbols and processes them in parallel. Comparing three stocks? One tool call, not three. Cuts latency 60-70%.

**Selective fields.** Most tools accept a `fields` or `sections` parameter. User asks about insider activity? Skip RSI, return just the insider data. Smaller responses, cheaper tokens, faster inference.

**No hallucination surface.** Each tool returns structured data from real APIs. The agent can only reference numbers the tools actually provided. One line in the prompt — "ONLY report financial metrics present in tool results" — killed 90% of fabricated numbers. For finance, this is the constraint that matters most. A hallucinated revenue number in a stock analysis can cost someone real money.
...
For queries needing 3+ steps, the agent writes itself a plan using `todo_write` — a scratchpad it works through step by step:

**Step 1**: `filter_stocks` — `revenue_growth_qoq > 0`, `market_cap > 500M`, `change_percent_3m < 75`. Companies showing sequential revenue growth but still trading below recent highs.

**Step 2**: `get_financial_charts` on the top 5 for revenue and operating margins. Is the turnaround real, or a seasonal blip?
...
**Step 4** (parallel): `analyze_technicals` + `generate_catalysts` on the top 3. Chart cooperating? Upcoming events that could move the price?

**Step 5**: Synthesize. Structured thesis for each candidate. Bull/bear cases. Rank them.

For full portfolio creation, Bloom uses a multi-agent system. A router coordinates four specialists: search (find candidates), research (pull financials/news/ratings), evaluation (grade if research is sufficient), and portfolio (create weighted allocation). If the evaluation agent flags "missing valuation context on two tickers," the router loops it back to research. Up to 7 iterations.

Slow — 30-60 seconds for a full portfolio. But the evaluation step catches the most common failure mode: confident allocations based on incomplete research.

I'd rather be slow and right than fast and reckless.

## 224 Tests, Because "Seems Fine" Isn't a Methodology

Something that genuinely bothers me about AI product development: most teams ship on vibes. I've talked to founders who A/B test button colors for weeks but deploy prompt changes after a five-minute spot check.

For a weather app, maybe that works. For a finance app where people make real decisions with real money? No.
...
When I expanded from 12 tests to 363 (now consolidated to 224), the first full run humbled me.
...
The fix for all 99? Sixteen lines added to the prompt. Four removed. Each traced directly to a failure category. The fabricated metrics fix was one line: "ONLY report financial metrics present in tool results." The valuation depth fix: "Interpret metrics — explain what they mean for the investment case."

Twenty lines of prompt changes. I would never have found those problems by reading outputs manually. I would've shipped it, and some user asking about @CRWD would've gotten a wall of numbers with no interpretation.

## What Actually Mattered

**The prompt is the product.** Bloom's 2,000-token prompt encodes an investing philosophy, a voice, and behavioral constraints. Changing one line changes how tens of thousands of interactions feel. We spent a week on a database migration and changed a prompt line in five minutes — the prompt line had 10x the user impact. Treat your prompt like core business logic. It is.

**Small tools compose better than big ones.** Seventeen focused tools beats three powerful ones. The model picks from a clear menu better than it navigates a multi-purpose function. And small tools combine in ways I didn't design for — the agent figured out research chains I never anticipated.

**Evals are the new unit tests.** You wouldn't ship backend code without tests. Don't ship prompts without evals. Start with 10 test cases covering your most important flows. Your first real expansion run will find problems you didn't know existed.
